...
def generate_grid_points(box, grid_size=10):
    """
    在一个边界框内部生成一个均匀的网格点。
    """
...
def generate_box_boundary_points(box, num_points_per_side=10, offset=5):
    """
    在边界框外部周围生成一圈负向点。
    """
    x_min, y_min, x_max, y_max = box
    points = []
    # 上边框外侧
    for x in np.linspace(x_min, x_max, num_points_per_side):
        points.append([x, y_min - offset])
    # 下边框外侧
    for x in np.linspace(x_min, x_max, num_points_per_side):
        points.append([x, y_max + offset])
    # 左边框外侧
    for y in np.linspace(y_min, y_max, num_points_per_side):
        points.append([x_min - offset, y])
    # 右边框外侧
    for y in np.linspace(y_min, y_max, num_points_per_side):
        points.append([x_max + offset, y])
