...
if __name__ == '__main__':
    model_path = 'E:/zhuyunhao/RTDETR-main/runpolyp/polyp/train/rtdetr-r18.yaml_train_120/weights/best.pt'
    model = RTDETR(model_path) # 选择训练好的权重路径rtdetr-r18.yaml_train_120
    result = model.val(data='dataset/1/1.yaml',
                      split='test', # split可以选择train、val、test 根据自己的数据集情况来选择.
                      imgsz=640,
                      batch=8,
                      save_json=True, # if you need to cal coco metrice
                      project='runpolyp/polyp/val',
                      name='rtdetr-r18.yaml_train_120_test_ETIS',
                      )
    
    if model.task == 'detect': # 仅目标检测任务适用pip 
